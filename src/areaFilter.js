// ============================================================
// areaFilter.js — Area · Sort · Near Me · Sunny · Type filter
// ============================================================
...
export function filterCafes(allCafes, area = activeArea, sunnyOnly = activeSunnyOnly) {
  // Attach distanceMeters to every place
  if (userLat !== null) {
    allCafes.forEach(c => {
  ...
  if (activePlaceType === 'cafes') {
    result = result.filter(c => c.placeType === 'cafe');
  } else if (activePlaceType === 'libraries') {
    result = result.filter(c => c.placeType === 'library');
  }
...
export function getCafeCounts(allCafes) {
  ...
  if (activePlaceType === 'cafes') {
    filtered = allCafes.filter(c => c.placeType === 'cafe');
  } else if (activePlaceType === 'libraries') {
    filtered = allCafes.filter(c => c.placeType === 'library');
  }

  const counts = { all: filtered.length };
  AREAS.forEach(a => {
    if (a.id !== 'all') counts[a.id] = filtered.filter(c => c.area === a.id).length;
  });
  return counts;
}
...
export function getSunnyCounts(allCafes) {
  ...
  const sunny = filtered.filter(c => (c.sunScore ?? 0) > 50);
  ...
  return `
    <div class="filter-bar">
      ...
      <div class="filter-controls">
        <button class="type-btn ${activePlaceType === 'cafes' ? 'active' : ''}" data-type="cafes">☕ Cafes</button>
        <button class="type-btn ${activePlaceType === 'libraries' ? 'active' : ''}" data-type="libraries">📚 Libraries</button>
        <button class="type-btn ${activePlaceType === 'both' ? 'active' : ''}" data-type="both">✨ Both</button>
...
function renderPills(allCafes) {
  ...
  pillsContainer.innerHTML = AREAS
    ...
    .map(a => {
      ...
      return `
        <button class="area-pill ${a.id === activeArea ? 'active' : ''}"
                data-area="${a.id}"
                title="${count} places, ${sunny} sunny now">
...
function attachEvents(allCafes) {
  ...
  // Type, Sort and Radius buttons
  root.addEventListener('click', e => {
    // Place type buttons
    const typeBtn = e.target.closest('.type-btn');
    if (typeBtn) {
      activePlaceType = typeBtn.dataset.type;
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .type-btn, .sort-btn, .radius-btn, .sunny-toggle {
    ...
    .type-btn.active, .sort-btn.active, .radius-btn.active, .sunny-toggle.active {
    ...
    @media (max-width: 600px) {
      ...
      .area-pill, .type-btn, .sort-btn, .radius-btn, .sunny-toggle { padding: 5px 10px; font-size: 11px; }
    }
  `;
